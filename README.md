...
Professor: Prof. Dr. Roberto de Alencar Lotufo \
...
### 1 - Language Model - Self-Attention
...
### 3 - Language Model - Self-Attention and Causal Masking
...
### 4 - Language Model with Self-Attention and Causal Masking + LoRA
...
### 5 - CLIP - Multimodal Search via Embedding
...
This activity focused on implementing a ReAct (Reasoning and Acting) framework to solve complex, multi-step queries by combining the logical reasoning of Large Language Models with external tool usage. The primary objective was to build an autonomous agent capable of iteratively thinking, searching for information via a retrieval tool, and observing results to refine its final answer, specifically applied to a dataset of Brazilian legal and tax questions (such as IRPF regulations). By utilizing LangGraph to manage the agent's state and a FAISS vector store for document retrieval, the activity demonstrated that the ReAct strategy significantly improves the model's ability to handle "out-of-knowledge" queries compared to standard prompting. The results showed that the agent could successfully decompose intricate questions into smaller search tasks, verify findings through multiple iterations, and provide grounded, accurate responses, effectively minimizing hallucinations in technical domains.

### 9 - Multiagents

This activity focused on the development of a complex Multi-agent System using LangGraph to orchestrate specialized LLM agents for collaborative task solving. The primary objective was to move beyond a single-prompt architecture by creating a modular team consisting of four distinct agents—a Manager for orchestration, a Searcher for information retrieval, an Answerer for generating responses, and a Summarizer for synthesizing data—all powered by the gpt-4o-mini model. By defining a state-based workflow where agents communicate and hand off tasks, the system demonstrated a significant improvement in managing multi-step reasoning and ensuring factual grounding through dedicated search cycles. The final results showcased a robust, autonomous pipeline integrated into a Gradio chatbot interface, capable of accurately routing user queries, iteratively searching for evidence, and producing concise, verified summaries, effectively proving that multi-agent orchestration enhances the reliability and depth of LLM interactions.